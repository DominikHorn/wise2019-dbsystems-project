...
import {
  ...
  export async function insertListeneintrag(
    ...
    client?: PoolClient 
      ...
      if (client) {
        return client
          .query(QUERY_STR, [kandidat_id, wahl_id, regierungsbezirk_id, initialerListenplatz])
          .then(res => !!res && res.rows[0]);
      }
      return adapters.postgres.transaction(async client =>
        insertListeneintrag(kandidat_id, wahl_id, regierungsbezirk_id, initialerListenplatz, client)
      );
  }